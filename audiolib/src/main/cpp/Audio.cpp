        ...
        if(size>0){
          //  ALOGD("有数据了");
            ...
            if(ret==0){
                ...
                SwrContext *swrContext=swr_alloc_set_opts(NULL, av_get_default_channel_layout(OUT_CHANNEL_NB), OUT_FORMAT, OUT_SAMPLE_RATE, avFrame->channel_layout,

                                                          (AVSampleFormat)(avFrame->format), avFrame->sample_rate, 0, NULL);
                ...
                if(swrContext==NULL||swr_init(swrContext)<0){
                    ...
                    int per_number_sample= swr_convert(swrContext,&data,avFrame->nb_samples,(const uint8_t**)avFrame->data,avFrame->nb_samples);

                    int frame_size_by_byte=per_number_sample*OUT_CHANNEL_NB*av_get_bytes_per_sample(OUT_FORMAT);
...
void Audio::initSLES() {
      ...
      (void)result;
      result=(*muixObject)->GetInterface(muixObject,SL_IID_ENVIRONMENTALREVERB,&outEnvironmentalReverbItf);
    //  ALOGE("result==>%d",result);
      ...
      result = (*slEngineItf)->CreateAudioPlayer(slEngineItf, &playerObject, &slDataSource, &audioSnk, 1, ids, req);
      //初始化播放器
